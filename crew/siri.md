...
the *actual* surface (the names the book exposes, the kinds, the operators) must
...
- **Your concern:** the user-facing namespace + the docs that describe it. The
  names the book exposes (`(names ())`), the vocab in CLAUDE.md / blue.md / README /
...
The global namespace is whatever `(names ())` prints — every key in the **book**,
the initial globals. Today it leaks compiler internals that must NOT be user-visible.
Clean it to a deliberate vocabulary:

1. **Survey.** `(names ())` prints all book keys (the initial global namespace).
   Triage each: a real user-facing name (keep), or a compiler/runtime internal that
   leaked (must go).
2. **Mop as needed.** The egg already pulls every compiler-internal name — the book
...
3. **Pull down into `ev` from `prel` where possible.** A name that prel defines as a
   ...
   evaluator's closures) rather than as a book global — so it's not a surface name at
...
4. **Where prel bootstrap genuinely needs the global** (it can't be pulled into `ev`
   because the bootstrap references it as a book name), leave it global but **add it
