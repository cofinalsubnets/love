...
**Where you are (2026-06-17):** milestone 1 is DONE and **boot-verified on metal**.
The kernel was gathered out of the repo root into `port/kship/` (a ship in port).
The agent (`port/kship/kship.l`) is baked into the image and runs the perceive→
decide→act heartbeat on the real timer tick. Next up is milestone 2 — the
virtio-net driver + a `k_sources[]` socket slot.
