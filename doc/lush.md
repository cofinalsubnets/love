...
The list of words that may take it is deliberately short. A main only qualifies if running it here is the *same thing* as running it there, and three kinds fail that -- a main that quits (it would end its caller), one holding state a single run owns (a recursive `$(MAKE)`), and one that can block (a spawned `cat` or `sleep` is a child `^C` kills and `kill %1` can name; in here neither is true). **mooncc**(1) is the one that pays most, since a build spends its life calling it; the rest are the bounded file and text tools a build spells between compiles. The tools that read standard input when handed no file operand stay off it and wait on the interrupt and stdin story -- nothing about the image wake waits with them, because the lane below already carries it. That is why `gzip`, `gunzip`, `zcat` and `cpio` are absent: a filter with no file operand would drink the terminal in here.

Of those three disqualifiers only the last two are laws. Quitting is a property a tool can simply stop having, and the tree carries the door already: **kore**(1)'s applets raise `uleave` and their registration wraps them in `urun`, which hands a caller in this image a status charm where a process would have died -- and **mooncc**(1) is on this list at all because `moon-run` sits under a `moon-main` that only quits with what it answers. `tar`, `libra` and `fat` (with `mkfs.vfat` and `mkdosfs`) took that door and joined the list; `nm`, `ar`, `ld` and `as` were already through it and only wanted naming. **cook**(1) cannot follow: its recipe cards and option boxes are state one run owns, and a build long enough to want interrupting wants a pid for `^C` to name -- which is the fork lane's whole argument, and why `sh-oneline` below is the door it uses instead.
...
The external tools stay external; the builtins are the ones that must run inside the shell's own process: **cd**, **pwd**, **exit**, **exec**, **export**, **command**, **read**, **set**, **shift**, **unset**, **eval**, **.**, **wait**, **local**, **break**, **continue**, **return**, **jobs**, **fg**, **bg**, **trap**, **:**, **true**, **false**, **which**, **where**, **type**, **ev**. `return` inside a `.`-sourced file stops that file (the POSIX dot-return), which is how */etc/profile.d* guards bail early.

`exec` \[*command* \[*arg*...\]\] is the builtin whose redirections stay. With a *command*, this process becomes it: same pid, no fork, and no line after it ever runs -- which is what makes `exec lush --login "$@"` a wrapper script and not a second process sitting on the first. With no *command*, the redirections alone are applied and they are the shell's from there on, so `exec 3<config` or `exec >log` outlives the line that set it. A word nothing can be found for is `127` and a word that cannot be run is `126`; either ends a script, POSIX's rule for a special builtin, while an interactive shell says so and stays. What the shell holds for itself does not ride along: the five terminal signals it ignores go back to their defaults, and a `trap` armed over a signalfd releases both the mask and the fd, or the command would inherit a shell's deafness to `^C`. In **autonomous** mode a bare word naming a verb this binary carries execs this binary under that word, the same path a spawn of it would take.

`which` \[**-a**\] *name*... and `type` *name*... answer the same question `command -v` does, off the same verdict, and it is the ladder's own: a reserved word, a function, a builtin, then the word's binary. Both are builtins because PATH cannot answer it -- PATH has never seen a function, and in **autonomous** mode PATH does not decide the word at all. There a verb this binary carries is ours whatever PATH holds, and a spawn of it execs this binary, so that is the path the answer names: `which mooncc` says the running artifact and `type mooncc` says `mooncc is a verb of `*path*, on a machine with no mooncc installed and with an empty PATH. Under **-g** the question is PATH's again and the answers are PATH's. `which` prints one line a word, spelled as `command -v` spells it (a builtin, function or reserved word answers its own name), and **-a** adds every other PATH hit after it -- which is how an installed twin of our own name shows itself. `where` *name*... is zsh's word for that walk with the flag already on (`whence -ca`): every answer rather than the first. `type` prints a sentence a word. Both say `name: not found` on standard error for a word nothing answers and exit 1, sticky over several operands; `command -v` stays quiet on a miss, POSIX's way and the reason a configure-shaped probe wants it instead.
...
**love**(1), **kore**(1), **cook**(1), **sh**(1p). The lush sources live in *a/lush.l* of the love tree; *t/host/sh.l* is the executable gate.
