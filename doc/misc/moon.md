...
`core/holo/link.l` (our static linker) and `apps/moon/lib/` (our libc, math floor and machine
...
Anything without `-c` is a **link**, through `core/holo/link.l`.
...
Read it back without any binutils at all: `core/holo/elfsec.l`'s `(elfsec PATH ".comment")` answers the
...
target — AT&T on x64, ARM on a64, riscv, thumb — and `core/holo/gas.l` lowers it to the
