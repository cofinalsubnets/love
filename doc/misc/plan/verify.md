...
**the native uu prover (l/boot/uu.l) is target number one.** a property worth
...
| angle | home | fate |
|---|---|---|
| uu kernel + UniMath corpus | l/boot/uu.l, test/uu*.l | THE TARGET |
...
apps/lux/core.l is written in the new style (glued accessors, infix, !=,
...
the whole apps/lux/ also dropped the backtick list sugar for [..] (a comment-
...
- the lift is KIND-DIRECTED BY THE TOWER ITSELF. every expression has a level in
  ...
  `(vnet (vnum (vnet v)))` with no rule of its own. apps/lux/sigs.l is wm2uu's
...
- **tools/mx2uu.l** -- mx2coq's uu twin, reading THE TABLE (l/mx.l, the same
...
- gated by `make test_uumx` off the uu_corpus roster (regenerate + diff, so a
  l/mx.l edit with no refresh reddens), and exported: all 28 mx entries
