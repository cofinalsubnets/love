...
- **A — native equivalents.** Our code doing the package's job (kore, lush, cook, moon,
  holo, `apps/gz/gz.l`, `apps/tar/tar.l`). This is where nearly all the distance is.
...
| LFS | ours | state |
| --- | --- | --- |
| binutils | `l/holo/` — as, ld, ar+ranlib, nm, objcopy | byte-identical smokes vs GNU/llvm |
...
applets now can: **`chroot`** (`apps/kore/proc.l`, the root moved, the cwd carried
...
bash (`lush`) · sed · grep · diffutils (diff, cmp) · make (`cook`) · tar (`apps/tar/tar.l`,
...
the cheapest -- apps/gz/gz.l carries the numbers), and `gzip -d` reads one member per file
...
- **the admin layer** — shadow, e2fsprogs, kmod, iproute2, kbd. Three of its packages
  ...
  and the `love fat` verbs over `apps/fat/fat.l` (`make test_fat32`, mtools the
...
- **rung 0 — `find` and `awk` — BUILT** (`e015bf61`). Both are
  `apps/kore/` applets on the u-floor, both in `make test_kore`: awk 39 checks
...
- **rung 1 — `patch`, and the coreutils stragglers — BUILT.** Thirteen applets: `patch`
  (`apps/kore/patch.l`, unified diffs, `-pN -R -i -o --dry-run -s`, offsets and rejects),
  `expr` (`apps/kore/expr.l`, its own file because `:` rides the BRE engine), `od paste
...
- **rung 1d — the /proc family — BUILT.** Eight applets in `apps/kore/proc.l`, and **no
  ...
  * **a `'-x '` row in the gate's grep matrix was `grep -x FILE`** — a pattern and no
    ...
    now.
- **rung 2 — cpio, and the distro cuts itself — BUILT.** `apps/cpio/cpio.l` is the SVR4 newc
  wire (pack, unpack, scatter) over apps/tar/tar.l's own entries — the walk that fills them
  is about a file and not about a format, which is why the second wire is short — and
  `apps/cpio/cpiocmd.l` is `love cpio` (`-o -i -t`, `-H newc`, `-d -u -v`, `-F/-I/-O`,
...
- **rung 2b — `bc` — BUILT.** `apps/kore/bc.l`, in `make test_kore`. A number is
  ...
  commutativity, `|a%b| < |b|`, `r² ≤ n < (r+ulp)²`) is lawed in `apps/kore/law.l`.
