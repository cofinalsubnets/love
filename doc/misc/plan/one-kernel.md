...
- **the io.c port surface takes a charm as an fd. LANDED.** `see` `say` `put`
  ...
  The lanes are inle/fd.c's, beside `ai_fd_write_all`, and they go at the ROW
...
- **`stat` takes a charm as an fd. LANDED.** `fstat` without a new name, and
  ...
  ROW had no love-level caller at all (inle/image.c's two are the hosted
...
three lanes those kinds ride are inle/fd.c's, where the fd doors already live.
