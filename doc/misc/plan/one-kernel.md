...
`-Dai_tco=1` is redundant: `core/love.h:39` already defaults it to 1 and the
...
- **the io.c port surface takes a charm as an fd. LANDED.** `see` `say` `put`
  ...
  The lanes are host/fd.c's, beside `ai_fd_write_all`, and they go at the ROW
...
- **`stat` takes a charm as an fd. LANDED.** `fstat` without a new name, and
  ...
  ROW had no love-level caller at all (host/image.c's two are the hosted
...
three lanes those kinds ride are host/fd.c's, where the fd doors already live.
...
`-DK_TEST -Dai_tco=1`, `inle/x64/asmops.h`'s last
...
it. That cost two fields in `inle/uefi/loader.c`'s hand-kept `struct k_boot` copy,
