...
- **the diagnostic obligation**: a non-constant `_Static_assert` is let by today, which is a
  ...
  second definition winning — gcc refuses it (6.9p3). It bit through `LvNif`, whose static is
...
**The table is generated, not maintained: `u/moon-parity.sh table` prints it and
`u/moon-parity.sh check` fails if this doc and the compiler have drifted** (`why` prints
