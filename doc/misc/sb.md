...
([`a/sb/merge.l`](../../a/sb/merge.l)) — disjoint edits to one file both survive, and
...
imported is `sb` (`a/sb/`, `(borrow 'sb)`, `.sb/`, `make test_sb`); *svalbard* is the prose name,
...
`a/sb/` + `lib/sb/` (the holo/kore all-the-way-down precedent). `sync`/`record` are the
