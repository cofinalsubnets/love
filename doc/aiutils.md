...
1. **the regex engine + grep** -- the meaty rung. a small POSIX-BRE-ish matcher
   as its own lawed engine file (the myers/patience precedent: engine + laws +
   fuzz first, tool surface after); then grep (-n -v -c -l) over it.
2. **sed-lite** -- s/re/repl/[g], d, p, -n, line/range addresses; rides the
   same engine.
3. **the process tools** -- env, sleep, kill, xargs. no new nifs: environ/
   setenv/getenv, `rest`, bao's `still`, spawn/wait.
4. **polish, as need arises** -- ls -l (stat already carries size/mtime/mode),
   cp -r, multi-source cp/mv into a directory, sort -n/-k, uniq -d/-u, cut -b,
   tr [:class:] and -ds, echo -e, seq over gems. none block the distro; add
   them when a real script wants them.
