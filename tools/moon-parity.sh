...
# all. Cross-target drift is the failure this tree actually ships -- love/love.c compiles
# everywhere, so the gaps live in the C that love/love.c never writes.
#
...
# (love/holo/link.l's ld-syms) behind nm's surface -- one wake over every object the
