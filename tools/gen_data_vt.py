#!/usr/bin/env python3
"""gen_data_vt.py -- bake the data-sentinel stride into a generated data_vt.h.

gwen recovers a heap object's kind from its `ap`: the five self-quote sentinels
(g/data.c, DATA_SENTINEL) are pinned contiguously and in enum order into the
.gwen_data_vt section, so a kind is just a slot index into that section --
g_typ(o) == (ap - start) / unit, unit == (stop - start) / N (see g/data_vt.h).
...
But the sentinels live in their own translation unit (g/data.c -> data.o), and
...
reflects on data.o, recovers that stride, and emits a data_vt.h that hard-codes
...
Usage: gen_data_vt.py <data.o> [-o data_vt.h]   (writes stdout if no -o)
"""
...
#ifndef _g_data_vt_h
#define _g_data_vt_h
// GENERATED by tools/gen_data_vt.py from data.o -- do not edit by hand.
//
// Same contract as the portable g/data_vt.h, but the sentinel stride is baked
...
#define G_DATA_VT_UNIT %du   // sentinel stride in bytes, from data.o
...
def main():
    ...
    if "-o" in args:
        ...
        del args[i:i + 2]
    if len(args) != 1:
        fail("usage: gen_data_vt.py <data.o> [-o data_vt.h]")
