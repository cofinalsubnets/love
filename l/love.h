...
// public so a host nif can wrap a C struct's bytes (i/cb.c).
...
// the horn: PCM out as a heap port (i/horn.c). it wears the fd port's shape --
...
// heap in hand (i/doom.c reaches k_fb the same way). 16-bit stereo, the rate
// rides the open: open -> 0 | -1 no device; write -> bytes landed, 0 full, -1 gone;
// lag -> frames queued and unplayed. inle's i/hda.c is the body; horn.c
...
// card otherwise (i/horn.c keeps that one open). what i/doomsnd.c calls.
...
// the raw-fd lanes (i/fd.c): what an io op does when its operand is a charm
...
// the seat's other doors, one definition each: i/posix.c..
...
// ..l/gz.c, and i/src.c's own source (weak zero without a blob)
...
// output only, and NULL asks for none -- i/mps2 prints them where there is no debugger.
...
// directly (i/main.c static-asserts the match). the scheduler fills .fd/.events and
...
// the two park directions, in poll(2)'s own bit values (i/main.c static-asserts
...
lvm_t lvm_kcall,
 ...
 lvm_eq0, lvm_lamparts,   // the leaf half of the walk, and a function value's parts: love's door into `=`
...
// silently breaking threaded dispatch (u/vmret.l).
