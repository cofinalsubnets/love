...
# all. Cross-target drift is the failure this tree actually ships -- l/love.c compiles
# everywhere, so the gaps live in the C that l/love.c never writes.
#
...
check)
  ...
  if diff -u "$d/doc.md" "$d/measured.md" > "$d/drift.txt"; then
    ...
    echo "             -doc  +measured; regenerate with 'u/moon-parity.sh table'."
