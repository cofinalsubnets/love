...
#   u/moon-sweep.sh PKGSRC=$PWD/gzip-1.13
#
...
if [ ! -f "$PKGSRC/lib/Makefile" ]; then
  ...
  echo "            set PKGSRC=<a ./configure'd tree with lib/> to run (see u/moon-sweep.sh)."
  exit 0
fi
...
# name a symptom and not a cause -- that gap is what closes.
kind() { cause "$1" |
