...
if [ -z "$LUASRC" ] || [ ! -f "$LUASRC/src/lua.c" ]; then
  ...
  echo "          set LUASRC=<an extracted lua-5.4.x tree> to run (see u/moon-lua.sh)."
  exit 0
fi
...
for f in "$LUASRC"/src/*.c; do
  b=$(basename "$f" .c)
  [ "$b" = luac ] && continue
  $MC $tflag -Iapps/moon/include -I"$LUASRC/src" -c "$f" "$d/$b.o" || { echo "FAIL mooncc -c src/$b.c"; exit 1; }
  objs="$objs $d/$b.o"
done
