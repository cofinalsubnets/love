...
static struct ai *ai_ini_0(struct ai*g, uintptr_t len0, void *(*al)(struct ai*, void*, size_t)) {
 ...
 if (ai_ok(g = map_new(g)) && ai_ok(g = map_new(g)) && ai_ok(g = ai_have(g, 9))) {
  ...
  struct ai_def def0[] = {
   ...
   // mopped at birth like every other raw pointer the compiler folds (src/core/boot/egg.l)
   {"ci-vt", (word) &ai_ci_vt},
   {"to-vt", (word) &ai_to_vt},
  ...
  // is registered under. A NOM, in the prel's canonical spelling (src/core/boot/prel.l's arch-canon)
...
// non-executable and keeps a second window that runs. inle does (src/inle/kmain.c).
...
char *code_adopt(struct ai *g, char const *src, size_t n) {
 // inle: mmap hands back the hhdm, which is NX by construction (src/inle/mkboot.l puts the
...
static ai_inline intptr_t seq_byte(word x) {
 ...
 return -1; }
// list lane. the matrix routes only list-involved pairs here (src/core/mx.l's five cells),
