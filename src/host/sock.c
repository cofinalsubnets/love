// src/host/sock.c -- every socket nif, both address families: TCP/UDP (ain's netcat core and
...
// so names resolve one layer up in love, where a lookup can park -- src/apps/dns/dns.l's `dial`.
