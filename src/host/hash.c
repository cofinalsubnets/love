// src/host/hash.c -- digests over a string's bytes. auto-globbed and AiNif-registered,
...
// src/apps/kore's cksum, md5sum and sha256sum applets are these four plus a line of output.
// they do not all stand on the same footing. crc32 shadows src/apps/gz/gz.l's gz-crcwalk and
...
// md5 shadow nothing, yet src/apps/sb's blob and patch ids and src/apps/moon's cache key rest on
...
// src/apps/gz/gz.l spells is a dependency chain one link per byte, where slicing spends eight
