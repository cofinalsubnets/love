...
// ai_clock lives in src/host/posix.c, one body for this frontend and the kernel's.
...
static void first_boot(char const **argv) {
  if (ai_srcgz_len < 18) return;                     // src/host/src.c's weak zero: this link carries no source
  ...
  if (!t) {                                          // truncated or not gzip; or the mmap failed
    ...
    return; }
  // per-process, for the reason the bake's scratch is (src/host/image.c): concurrent first
