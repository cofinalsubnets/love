/* src/apps/moon/lib/nolibc/num.c -- strtol/strtod, and the stdio odds and ends the
...
double ldexp(double x, int n) {                    /* x * 2^n, clamped through the rim in steps */
  ...
  return x * __e2d(n); }
/* the math floor's exact reader (src/apps/moon/lib/math/am.c -- linked wherever
 ...
 * promised. every one of these was declared in src/apps/moon/include/ with no body
