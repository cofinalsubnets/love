...
the rune CAS (src/apps/rune/) riding the crank. crank (or left/right) picks a
...
the 8x8 CGA font, blitted to the 1-bit LCD each frame).
    ...
    make -C src/port/playdate        # out/playdate/love.pdx (device + simulator)
    make -C src/port/playdate sim    # run it in the Playdate Simulator
