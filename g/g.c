  ...
  case '"': {
   ...
   for (size_t lim = sizeof(word); g_ok(f); f = grbufg(f), lim *= 2)
    for (struct g_vec *b = (struct g_vec*) f->sp[0]; n < lim; txt(b)[n++] = c) {
     if ((c = i->getc(f, i)) == '\\') {             // escape: take next char
      if ((c = i->getc(f, i)) == EOF) return encode(f, g_status_more); }
...
static int _li_eof(struct g *f, struct li *i) {
 return i->peek < 0 && !twop(i->l); }
static int _li_getc(struct g *f, struct li *i) {
 if (i->peek >= 0) { int c = i->peek; i->peek = -1; return c; }
 if (!twop(i->l)) return EOF;
 int c = getnum(A(i->l));
 return i->l = B(i->l), c; }
static int _li_ungetc(struct g *f, int c, struct li *i) {
 return i->peek = c; }
...
struct g *g_feed(struct g *f) {
 for (int c; g_ok(f) && (c = ggetc(f)) != EOF; ) f = g_edit(f, c);
 return f; }
