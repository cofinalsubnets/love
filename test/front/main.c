...
//   (tapped ())   the PCM inle/horn.c's sink accepted, as text; (taprate ()) and
...
// well as the vtable: inle/fd.c has them on a hosted seat and inle/fdrow.h on a
...
// --- the horn's tap ---------------------------------------------------------
// inle/horn.c's sink hands its ACCEPTED frames to ai_horn_tap and the weak default in
...
//   (tapchans ())  ..and the channel count. inle/horn.c doubles a mono port on the way
...
// ..and the SEAT's door, the OTHER way PCM leaves love: inle/horn.c asks k_horn_* where the
...
//
// and it is the DEVICE'S OWN RING under them -- inle/hornring.h, the same header
// inle/playdate/pdglue.c hands its SDK callback. that ring is the one part of a seat's
...
// speaker is 44100 alone), and inle/horn.c's misuse laws already cover the refusal path.
...
int main(int argc, char const **argv) {
  ...
  // ..and the LvNif slice of every TU linked beside this one, as inle/main.c drains it:
