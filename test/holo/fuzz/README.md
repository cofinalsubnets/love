...
The verification frontier stops at holo today: `love/holo/` (the assembler -- x64, a64, rv64,
...
- **a64**: encoding `31` is SP only in load/store-base and add/sub-immediate contexts; as a
  ...
  uses it *only* as SP (per `love/holo/a64.l`), so `(cmp x sp)`, `(jmpr sp)`, a loaded/stored
...
It reads the op tables straight out of `love/holo/a64.l` (`a64-sysregs`, `a64-pstate`,
`a64-barrier-opts`, and the four SYS tables) and riscv's CSR table out of `love/holo/rv64.l`,
...
(see `love/holo/x64.l` — `trap` is the `CC` form).
