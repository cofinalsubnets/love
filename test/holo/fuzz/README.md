...
The verification frontier stops at holo today: `core/holo/` (the x86-64 + a64 assembler)
...
- **a64**: encoding `31` is SP only in load/store-base and add/sub-immediate contexts; as a
  ...
  uses it *only* as SP (per `core/holo/a64.l`), so `(cmp x sp)`, `(jmpr sp)`, a loaded/stored
...
It reads the op tables straight out of `core/holo/a64.l` (`a64-sysregs`, `a64-pstate`,
...
(see `core/holo/x64.l` — `trap` is the `CC` form).
