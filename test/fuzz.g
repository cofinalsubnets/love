...
(:
 ...
 (fz-pure v) (\ st (X v st))                          ; ignores the stream
 (fz-map f g) (\ st (: r (g st) (X (f (A r)) (B r))))
 (fz-bind g k) (\ st (: r (g st) ((k (A r)) (B r)))) ; feed value into next gen
...
(assert
 ...
 (fz-ok 400 1 (fz-pair2 fz-num fz-num) (\ v (= (+ (A v) (AB v)) (+ (AB v) (A v)))))
 (fz-ok 400 2 (fz-pair2 fz-num fz-num) (\ v (= (* (A v) (AB v)) (* (AB v) (A v)))))
 ...
 (fz-ok 300 7 (fz-pair2 fz-num fz-num) (\ v (= (- (A v) (AB v)) (- 0 (- (AB v) (A v))))))
...
(: fz-arrpair (fz-bind (fz-int 1 6) (\ n (\ st
    (: a ((fz-rep n fz-elem) st) b ((fz-rep n fz-elem) (B a))
       (X (L (arrl i64 (L n) (A a)) (arrl i64 (L n) (A b))) (B b)))))))
(assert
 (fz-ok 250 40 fz-arrpair (\ v (aall (= (+ (A v) (AB v)) (+ (AB v) (A v))))))  ; + commutes
 (fz-ok 250 41 fz-arrpair (\ v (aall (= (* (A v) (AB v)) (* (AB v) (A v))))))  ; * commutes
 ; sum is linear: sum(a+b) = sum(a) + sum(b)
 (fz-ok 250 42 fz-arrpair
   (\ v (= (asum (+ (A v) (AB v))) (+ (asum (A v)) (asum (AB v)))))))
