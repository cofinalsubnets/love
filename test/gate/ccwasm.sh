...
# inle/wasm/run.mjs (the loader is the kernel), and required to answer exactly what the
...
# the OPTIONAL extra oracle, found as inle/wasm/Makefile finds it
...
for f in test/cc/*.c; do
  ...
  timeout 60 "$NODE" inle/wasm/run.mjs "$d/$b.wasm" > "$d/$b.tout" 2>&1; rt=$?
