...
{ echo "(use 'holo)"
  ...
  cat apps/moon/floor.l apps/moon/lex.l apps/moon/cpp.l apps/moon/parse.l \
      core/holo/text.l core/holo/dialect.l core/holo/gas.l apps/moon/val.l apps/moon/gen.l
...
echo "CC core/holo/text.l (love0 lane)"
"$love0" -l core/holo/text.l -e '(? (two? ((from (name "holo") (name "asm-text")) "li r0, 60")) (quit 0) (quit 1))' </dev/null \
...
# love2 differ and name a broken fixpoint (core/holo/link.l says it at the door).
# read ./VERSION rather than writing 0.1 down -- a release bump must not fail here.
cmt() { "$m" -l core/holo/elfsec.l \
