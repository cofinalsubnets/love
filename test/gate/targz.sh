#!/bin/sh
# test/gate/targz.sh -- apps/tar.l + apps/gz.l against the two programs they replace.
#
...
cat apps/gz.l apps/tar.l                  > "$w/tree/text.l"
...
# ---- 4. the command faces: gzip, gunzip, zcat (apps/gz.l) --------------
...
c="$w/cmd"; mkdir -p "$c"
head -c 20000 /etc/services > "$c/f.txt" 2>/dev/null || cat apps/gz.l > "$c/f.txt"
cp "$c/f.txt" "$c/g.txt"
chmod 0640 "$c/f.txt"
...
echo "targz: apps/tar.l + apps/gz.l agree with GNU tar and GNU gzip both ways -- ok"
