...
// which libc is under us. mooncc links moonlibc, which carries every door on all three
// kernels (apps/moon/include/sys), so a callable door is a build fact here and a box fact
// under a foreign libc -- what inle/posix.c's ladder is written against; the ambient-cc lane
// (love0, glibc) takes that ladder's kernel arms.
...
// the pack/call/unpack most nifs wear: hand the stack to a C helper that may collect, ghelp
...
struct ai {
 ...
 union {
  ...
  struct {
   word
     stack,  // the books checked out, ours on top; the macro table is stack[zero]'s, and
   ...
   union {
    ...
    struct ai_io {
     ...
     // prel's tap/jug poke this layout by index (love/boot/prel.l): a word added renumbers there
    } *io; }; }; };
 intptr_t end[]; };
...
// declaration does not answer it: ld leaves a weak undefined at 0 even where the section is
// there, unregistering every host nif in silence -- an embedder owning no LvNif defines the
// empty pair itself. the alignment is load-bearing: the bracket is read as an array, so a
// padded entry leaves the drain walking a stride nothing is laid at. a literal, not
// _Alignof: mooncc reads this attribute's operand as a number token only.
...
// writen discards). neither blocks the scheduler; the generic layer above owns ungetc_buf.
//   writen: land up to n bytes in one motion, count in g->b: >0 landed, 0 no room now
//     (caller keeps the residue), -1 the device is gone. it may allocate, so land nothing
//     after an allocating step. only a port that keeps a write run may refuse.
...
// unadvanced, `return Ap(lvm_yield_sw, g)` -- the op re-runs on reschedule.
      lvm_yield_sw,
// how a frontend nif ends the running task: stack down to one word (the retval), Ip at a
// {lvm_task_exit} cell, `return Ap(lvm_task_exit, g)` -- catch reads the retval off the node.
      lvm_task_exit;
...
// full, -1 gone; lag -> frames queued. inle/hda.c is the body, horn.c the weak refusals.
...
// ASLR delta shifts them all); a JIT W^X page, an mmap or a shared library dies with the
// bake process, so the dump refuses it.
// a refused dump names its first offenders: (heap word offset, the value, that object's ap);
// output only, NULL asks for none. `why` names the step that refused: 2 the compaction
...
// enum q, the value-kind lattice for generic dispatch: KMint the blue floor, then KNom, the
// arithmetic lane [KCharm..KTrayO] (scalars then their tray mirrors), the sequence lane
...
// ai_kind_of_d is the one crossing; both rosters are mx.l's -- edit that, not kinds.h.
#include "kinds.h"
...
// object layouts, their predicates and accessors, and the seam between love/*.c's
// translation units. above this line is what a nif writer needs.
...
// see later inserts. both are plain threads; empty slots hold map_gap, a unique out-of-pool
...
// eqv is declared here, not with the other bools at the tail: eql below calls it.
...
// a fairness yield clears any stale one-shot park intention first: lvm_fgetc never clears the fd
...
// --------------------------------------------------------------------------
// the TU seam. love/*.c is one runtime cut into translation units; everything not named
// here stays static to its own file. an always_inline that crosses loses the attribute.
// --------------------------------------------------------------------------
