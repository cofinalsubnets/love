...
// heap in hand (inle/doom.c reaches k_fb the same way). 16-bit stereo, the rate
// rides the open: open -> 0 | -1 no device; write -> bytes landed, 0 full, -1 gone;
// lag -> frames queued and unplayed. inle's inle/hda.c is the body; horn.c
...
// defines it -- inle/alloc.c is that answer over malloc and free, linked by a seat whose
...
// output only, and NULL asks for none -- inle/mps2 prints them where there is no debugger.
...
# endif
#endif
// LvBakeSrc -- this link can bake the crew from the source it carries (inle/src.c's
...
// the instruction table (love/snap.c): an instruction word <-> its index. a negative index
// is not an instruction, address 0 is no such index. ai_nif_cell (love/love.c) is the line
