...
// does not carry. per-process, for the reason the bake's scratch is (inle/image.c).
static int bsrc_lay_cat(char *cat, size_t n, char const *at) {
  char exe[4096];                                    // the kernel's own PATH_MAX, not a cap of ours
  if (ai_srcgz_len < 18) return 0;                   // inle/noblob.c's zero: this link carries no source
