// inle/wasm/repl.js -- the repl island: moon's wasm module driven from a page --
...
async function loveRepl(root) {
  ...
  // quay: the page's love side (inle/wasm/web.l: the tty words this seat has no
  // device for), then the real engines (apps/rove, ink) -- a closure captures its
  ...
  try {
    const srcs = await Promise.all(
      ['apps/rove/story.l', 'apps/rove/levels/lighthouse.l', 'inle/wasm/web.l', 'apps/rove/rove.l', 'apps/ink.l']
        .map(source));
    ev(srcs[0]);
