...
#include "pd_api.h"
#include "../../love/love.h"                   // ai_alloc, the runtime's one heap door
#include "pdglue.h"
#include "../hornring.h"
...
// wants -- and the horn PUSHES interleaved stereo. inle/hornring.h is the ring between
// them, shared with the test frontend so the awkward half is lawed off the device
// (test/front/hornseat.l); what is here is only the SDK's own shape.
//
...
// inle/alloc.c. (love/love.h names the door; inle/alloc.c is what a malloc-heap seat links.)
