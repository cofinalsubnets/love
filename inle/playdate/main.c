...
// this roster says ai_horn_seat, so inle/horn.c asks these instead of hunting a kernel's
// device -- the same four inle/hda.c answers under inle. the ring and the SDK source are
// pdglue's, which owns pd_api.h; nothing but words crosses.
//
// a mono port is doubled to stereo before it reaches here (inle/horn.c's horn_land), so
...
void love_init(void) {
  ...
  for (char const *s = woke ? "; inle/playdate -- image awake"
                            : "; inle/playdate -- baking the egg"; *s; s++)
    cb_putc(kcb, *s);
  blit();
  ...
  // inle/horn.c's rows ride the section, not the table above.
