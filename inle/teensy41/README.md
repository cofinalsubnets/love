...
```
cd inle/teensy41 && make            # -> $R/out/teensy41/love.hex
...
entry); the qemu cousin `inle/mps2/` proves the same runtime end to end under
...
in `love/boot/post.l` drives it exactly as it drives the kernel's. USB CDC (so the
