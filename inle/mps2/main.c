...
// libc + this file all compile -t thumb2 (inle/mps2/Makefile); only start.S
...
int main(void) {
  sh_puts("\n; inle/mps2 waker -- cross-binary wake\n");
...
;
int main(void) {
  sh_puts("\n; inle/mps2 baker -- baking the corpus\n");
...
int main(void) {
  uart_init();
  for (char const *s = "\n; inle/mps2 -- baking the egg on the M7\n"; *s; s++)
    sh_putc(*s);
  freelist = (struct mem*) POOL;
  freelist->next = NULL;
  freelist->len = POOL_BYTES / sizeof(uintptr_t);
