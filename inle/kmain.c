...
uintptr_t khhdm;
// the window that runs: the hhdm carries NX for the whole higher half (inle/mkboot.l), so
...
// the console's face. the palette that goes with it lives in love/quay/paint.c, which
...
// translate. inle/sys.c's raw-fd door names absolute rows for the same reason.
...
// inle/sys.c's door: the POSIX shapes over the same rows. the port layer says end with -1 and
...
// a seat that can sleep finer than the tick (inle/wasm's nanosleep) takes a short sleep
...
// machine's own names -- inle/rootfs/, a second tar walked with no prefix -- and a home, where
// the shell starts.
...
// overrides the weak nothing here and k_fs_init lays a row apiece (inle/doom.c's WAD is the
...
// failure to -1 / (), so the reasons inle/sys.c's openat needs are told apart here and
...
// have to invent -- ino, nlink, uid and dev are inle/sys.c's fabrication to make in the open,
...
// character device. the struct stat fabrication stays inle/sys.c's.
...
// the driver is inle/blk.c (virtio-blk, polled, synchronous), and DMA rides the love string's
...
// of this one -- a seat whose machine is a page's worker can (inle/wasm); metal cannot yet.
...
// ..and its Intel twin (inle/x64/vmx.c). (vmx-run ())
...
// inle/sys.c hands these answers straight out with no flip. the love side wears the same
...
void kmain(void) {
 ...
 // arming rides with it: what a hosted start would set (inle/sys.c).
 __ai_osv = -1;
 k_seat_init();
 khhdm = kboot.hhdm;
 archinit();
 ...
 // looked for on the registry: `love test/kernel/all.l` names the corpus from any seat,
