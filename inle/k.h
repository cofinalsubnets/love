...
// Combines the former inle/k.h (kernel logging) and free/k_boot.h (the boot
// hand-off struct).
#pragma once
#include "love.h"
...
// the sound card (inle/hda.c): probe at boot with one DMA block, and the poll the
// idle wait rides -- it reads the play position and silences what has played
void k_hda_init(void *dma), k_horn_poll(void);

// the arch backend (<a>/arch.c) gives kmain these
...
// the x64 virtualization doors (inle/x64/svm.c, inle/x64/vmx.c)
