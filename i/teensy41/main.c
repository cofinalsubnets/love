...
// the readiness law (i/main.c, inle's kmain.c): a NEGATIVE fd is ALWAYS
...
int main(void) {
  ...
  // i/mps2's baker -- fully symbolic, so this differently-linked binary
...
#include "post.h"
  ...
  g = ai_open_(g);
  // the seat laws (i/seat.l) answer first: a shell over an image that fails them is
