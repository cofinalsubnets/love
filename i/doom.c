// i/doom.c -- doom on both seats: the doors doomgeneric asks a platform for,
...
// for it (kmain.c's hook), so doom's own fopen/fread reach it through i/sys.c
...
static void doom_run(void) {
 memcpy(dh.wad, "/usr/share/doom1.wad", 21);   // the baked row
