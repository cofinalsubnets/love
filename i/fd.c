// i/fd.c -- love's ports over OS file descriptors: ai_fd_port_vt and the ai_fd_* family
// beneath it, plus the readiness and wait primitives the scheduler parks on. the bare-board
// counterpart is i/fdrow.h, which this file is poll.h and signal.h deeper than.
// i/main.c's binary and the inle kernel both link it, so what lives here exists once
// rather than twice. the bodies bottom out in libc calls, and on inle those land in
// i/sys.c's arms (__ai_call's negative-osv door), so most need no branch of their own.
#include "love.h"
#include <errno.h>
#include <fcntl.h>
#include <poll.h>
...
// the kernel's port lanes (i/kmain.c), reached on a negative osv: a protocol read(2)
