...
uintptr_t khhdm;
// the window that runs: the hhdm carries NX for the whole higher half (i/mkboot.l), so
...
// the seat hooks i/fd.c branches to on a negative osv (weak no-ops there)
void k_row_close(int fd), k_sleep(uintptr_t ms), k_wait_fds(struct ai_wait_fd*, int, uintptr_t),
     k_seat_init(void);                // i/sys.c: arm environ + the std streams
...
// the baked-image door (i/image.c): a pure read off two symbols the
...
// task asks after a key -- a guest that never idles still hears one -- and told the
// room the queue has, so a burst waits in the seat's own buffer rather than falling
// out of this one. metal has nothing to do.
...
// the port lanes ai_fd_port_vt (i/fd.c) takes on a negative osv: the seat
...
// i/sys.c's door: the POSIX shapes over the same rows. the port layer says end with -1 and
...
// ai_fd_close's inle lane (i/fd.c): close through k_sources[fd].
...
// overrides the weak nothing here and k_fs_init lays a row apiece (i/doom.c's WAD is the
...
// failure to -1 / (), so the reasons i/sys.c's openat needs are told apart here and
...
ai_noinline int k_fs_open(char const *p, uintptr_t pn, char m) {
  ...
  *s = (struct k_source) { .readn = ram_readn, .writen = ram_writen,
  ...
  return fd; }
// the open/close nifs are i/posix.c's now (plan C2): its open(2)/close(2)
// land in i/sys.c's arms, so the ramfs answers the same door -- and a
...
// have to invent -- ino, nlink, uid and dev are i/sys.c's fabrication to make in the open,
...
ai_noinline static int k_dup_row(int src, int at) {
  ...
  *t = *s;
  ...
  return fd; }
// i/sys.c's doors over the same motions: fcntl's F_DUPFD (at = the floor)
...
// the row mechanics of pipe(2), g-free: two rows over one queue. i/sys.c's
...
// --- directory rows: opendir(2)'s door, i/sys.c's only caller ------------
...
// character device. the struct stat fabrication stays i/sys.c's.
...
// TASK pid: i/main.c's getpid nif branches here on a negative osv, where
...
// --- the SVM spike (x64 only; i/x64/svm.c). (svm ()) is the capability and
...
// ..and its Intel twin (i/x64/vmx.c). (vmx-run ())
...
// i/sys.c hands these answers straight out with no flip. the love side wears the same
...
// i/main.c's quit nif branches here on a negative osv: the task/machine door.
lvm(k_lvm_quit) {
  if (k_task_exit(g)) {
...
static struct ai_def const __attribute__((section("ai_knifs"), used)) defs[] = {
  ...
  {"key", {.k = nif_key}},
  {"fault", {.k = nif_fault}},
  // the posix surface, open/close/quit/getpid included, is i/posix.c's and i/main.c's,
...
void kmain(void) {
 ...
 // arming rides with it: what a hosted start would set (i/sys.c).
 __ai_osv = -1;
 k_seat_init();
 khhdm = kboot.hhdm;
 archinit();
