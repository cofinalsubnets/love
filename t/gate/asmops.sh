#!/bin/sh
# t/gate/asmops.sh -- the inline-asm SEAM gate.
#
# i/<a>/asmops.h says every privileged instruction the kernel needs
# ONCE, in GNU's template: clang reads it natively, mooncc lowers the same text
# through l/holo/gas.l. one spelling, two readers -- and the reader that
# rots quietly is ours, so:
#
#   1. COVERAGE. every `static inline k_*` the header defines is called by
#      t/gate/asmops.c. derived from the header itself, so adding an op and
...
# header, and the encodings themselves are frozen in t/holo/golden.l.
#
# usage: asmops.sh HOSTDIR
set -u
ho=$1
probe=t/gate/asmops.c
