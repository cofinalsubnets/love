#!/bin/sh
# t/gate/targz.sh -- a/tar.l + a/gz.l against the two programs they replace.
#
# t/host/gz.l proves the laws that need nothing outside the tree: crc32 against
...
cat a/gz.l a/tar.l                  > "$w/tree/text.l"
...
# ---- 4. the command faces: gzip, gunzip, zcat (a/gz.l) --------------
...
c="$w/cmd"; mkdir -p "$c"
head -c 20000 /etc/services > "$c/f.txt" 2>/dev/null || cat a/gz.l > "$c/f.txt"
cp "$c/f.txt" "$c/g.txt"
chmod 0640 "$c/f.txt"
...
# ..and again from somewhere else, which is the half standing in $c cannot see: the
# stored name is restored BESIDE the archive, as GNU does, not into the cwd
mkdir -p "$c/nd" "$c/elsewhere"
cp "$c/g.txt" "$c/nd/stored.txt"
"$love" gzip -N "$c/nd/stored.txt" || fail "gzip -N"
mv "$c/nd/stored.txt.gz" "$c/nd/moved.gz"
( cd "$c/elsewhere" && "$r/$love" gunzip -N "$c/nd/moved.gz" ) || fail "gunzip -N elsewhere"
[ -e "$c/nd/stored.txt" ] || fail "gunzip -N did not restore beside the archive"
[ -e "$c/elsewhere/stored.txt" ] && fail "gunzip -N dropped the file in the cwd"
...
echo "targz: a/tar.l + a/gz.l agree with GNU tar and GNU gzip both ways -- ok"
