#!/bin/sh
# fat32.sh -- `love fat` and `love mkfs.vfat`, the command line over a/fat/fat.l.
#
# NOT t/gate/fat.sh, which is a different subject entirely: that one gates the
# FAT CONTAINER (one file, many architectures) and
# has nothing to do with the filesystem. `make test_fat` is that gate; this is
# `make test_fat32`.
#
# t/fat.l proves the FILESYSTEM against its own laws over a cask; this gate
