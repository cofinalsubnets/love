#!/bin/sh
# t/gate/fat.sh -- the fat container: the ONE
...
# the member list MIRRORS the Makefile's dist-fat recipe (the drift trap).
"$boot" u/fatpack.l "$d/fat2" "$a" "$ho/love" "$xa" "$xd/love" >/dev/null \
  || fail "repack"
