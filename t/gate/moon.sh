...
# `return 1` from a T* is a constraint violation -- i/main.c carried one for years,
...
# would rename the object, and dropping it renames it in silence. t/cc/145 holds the
...
# it found. t/cc/138 holds the well-formed side; only the refusals live here.
...
# ..and in an IDENTIFIER, the other position a UCN takes (t/cc/138 holds the well-formed side)
...
moonrun            t/cc/112-alwaysinline.c "$ho/.ni-on"  > /dev/null 2>&1 || fail "-fno-inline: splice-on compile"
moonrun -fno-inline t/cc/112-alwaysinline.c "$ho/.ni-off" > /dev/null 2>&1 || fail "-fno-inline: splice-off compile"
"$ho/.ni-on";  a=$?
"$ho/.ni-off"; b=$?
[ $a -eq $b ] || fail "-fno-inline changed the answer (on $a, off $b)"
moonrun -c            t/cc/112-alwaysinline.c "$ho/.ni-on.o"  > /dev/null 2>&1 || fail "-fno-inline: -c splice-on"
moonrun -c -fno-inline t/cc/112-alwaysinline.c "$ho/.ni-off.o" > /dev/null 2>&1 || fail "-fno-inline: -c splice-off"
