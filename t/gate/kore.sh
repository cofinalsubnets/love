#!/bin/sh
# t/gate/kore.sh -- kore, the multi-call toolbox (a/kore/), against GNU coreutils
...
if [ "$(uname -m)" = x86_64 ]; then
  ...
  # ar writes it, our linker reads it (l/holo/link.l's ld-arsyms).
...
# tree does not carry.
sh t/gate/sortcmp.sh "$K" || fail "kore sort: the flag matrix diverges from GNU"
sh t/gate/lscmp.sh "$K"   || fail "kore ls: the flag matrix diverges from GNU"
...
# everywhere. This box is the linux arm of that; the inle arm is t/kernel/wfs.l.
...
# line to cli-line (l/boot/post.l), the very door an exec would have reached, so a word
...
# ..and the FACE, which needs a terminal: t/host/less.l tethers one and types at it
echo "PAGER t/host/less.l (the face on a pty)"
cat t/00-init.l t/host/less.l | LOVEBIN=$K LESSTESTDIR=$HO "$m" > "$o" 2>&1
r=$?
