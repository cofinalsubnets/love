#!/bin/sh
# t/gate/run.sh -- the corpus-gate shape, in one place: run a love over some source, keep
...
#
# /warn LOVE is a word LIST, not a path: an egg lane is `env LOVE_NO_IMAGE=1 b/love`,
...
# THE X IS THE OTHER HALF OF THE SENTINEL. t/00-init.l's `test` RECORDS a failure and
