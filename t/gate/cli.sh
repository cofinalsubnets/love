#!/bin/sh
# t/gate/cli.sh -- the CLI's EXIT STATUS, which nothing else pinned.
#
# l/boot/post.l's every failure lane ends in a status: 1 for a file it cannot open, 2
