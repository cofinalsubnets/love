#!/bin/sh
# t/gate/softfp.sh -- the compiler runtime (a/moon/lib/rt.c) against the hardware.
...
if command -v "$cc" >/dev/null 2>&1; then
  if "$cc" -O2 -w -I. -o "$d/cc" t/gate/softfp.c 2>"$d/cc.err"; then
...
if moonrun -I. t/gate/softfp.c "$d/native" > "$d/native.err" 2>&1; then
...
for t in a64 rv64; do
  ...
  moonrun -t $t -I. t/gate/softfp.c "$d/$t" > "$d/$t.err" 2>&1 \
