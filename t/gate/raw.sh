#!/bin/sh
# t/gate/raw.sh -- the GCC-FREE fixpoint, for one target. Everything test_selfhost
...
# (t/test.mk): the corpus TESTS stdin, and `reads` no longer drains stdin ahead of the
# first form, so a piped corpus has t/io.l's see/unsee poking the script it is riding on.
cat "$@" > "$ho/.corpus.l"
