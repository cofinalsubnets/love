#!/bin/sh
# t/gate/asrefuse.sh -- the OTHER half of the as.l gate. a golden can only say what the bytes
...
try() {   # try NAME SOURCE  -- SOURCE must RAISE
  n=$((n + 1))
  { cat l/holo/holo.l l/holo/x64.l l/holo/dialect.l l/holo/as.l
    printf "(borrow 'holo)\n(: _ (puts (as-hex \"%s\")) _ 0)\n" "$2"
  } | $love > /dev/null 2>&1 &&
...
echo "t/holo/asrefuse: $n refused, $leaked leaked"
[ "$leaked" -eq 0 ] || exit 1
