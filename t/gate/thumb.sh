#!/bin/sh
# t/gate/thumb.sh -- the 32-bit ARM codegen gates, on the DEVICE CPUs under qemu.
...
am=a/moon/lib/moonlibc/math/am.c
aminc="-Ia/moon/lib/moonlibc/math -Ia/moon/include"
...
case $tgt in
thumb1)
  lane v  t/thumb1/libv.c  t/thumb1/harnessv.c  "" 7  30 "thumb1 varargs" \
    " = every differential check vs gcc; 100+n names the first miss -- see t/thumb1/harnessv.c; the pop-r3/bx epilogue or the r0-r3 push block over lr/fp/r4 is the usual suspect"
  lane p  t/thumb2/lib64.c t/thumb2/harness64.c "" 48 30 "thumb1 64-bit pairs" \
    " = every differential check vs gcc; 100+n names the first miss -- see t/thumb2/harness64.c; the v6-M lanes ride ADCS/SBCS inline + __aeabi_lmul/shift + __u/divdi3 libcalls"
  lane d  t/thumb2/libd.c  t/thumb2/harnessd.c  "" 45 30 "thumb1 soft doubles" \
    " = every differential check vs gcc's base-ABI soft float; 100+n names the first miss -- see t/thumb2/harnessd.c; doubles ride gp pairs at every seam, f0/f1/f15 are frame cells inside a fn (soften6)"
  lane am "$am" t/thumb2/harnessam.c "$aminc" 9 60 "thumb1 am.c" \
  ...
  lane f  t/thumb1/libf.c  t/thumb1/harnessf.c  "" 7  30 "thumb1 bare floats" \
    " = every differential check vs gcc; 100+n names the first miss -- see t/thumb1/harnessf.c; a bare float is ONE WORD on v6-M (ai_flo_t IS float on a 32-bit love -- the widened-pair mismatch here kept the egg from hatching)"
  lane a  t/thumb2/liba.c  t/thumb2/harnessa.c  "" 6  30 "thumb1 aligned(N)" \
    " = every aligned(N) global lands on its N after the link; 100+n names the first miss -- see t/thumb2/harnessa.c. the pad inside a section is laid by mooncc either way, so a miss here is sh_addralign: objsecs3's data lanes handing the linker a grain narrower than the stream asked for"
  lane z  t/thumb1/libzn.c t/thumb1/harnesszn.c "" 9  30 "thumb1 composites" \
    " = every differential check vs gcc; 100+n names the first miss -- see t/thumb1/harnesszn.c; the MEMORY-return (sret) lane and the position-0 16B r0-r3 quad are the featured shapes"
  ...
  { echo "(borrow 'holo)"
    cat l/holo/thumb1.l a/kore/text.l a/kore/u.l
    echo "(borrow 'kore)"                 # ld32.l reads uread; the floors above register 'kore
    cat a/kore/asbook.l \
        l/holo/elf.l l/holo/obj.l l/holo/link.l t/gate/ld32.l
...
thumb2)
  lane p  t/thumb2/lib64.c t/thumb2/harness64.c "" 48 30 "thumb2 64-bit pairs" \
    " = every differential check vs gcc; 100+n names the first miss -- see t/thumb2/harness64.c"
  lane d  t/thumb2/libd.c  t/thumb2/harnessd.c  "" 45 30 "thumb2 VFP doubles" \
    " = every differential check vs gcc -mfloat-abi=hard; 100+n names the first miss -- see t/thumb2/harnessd.c"
  lane am "$am" t/thumb2/harnessam.c "$aminc" 9 60 "thumb2 am.c" \
    " = the seven transcendentals BIT-IDENTICAL to the host am floor, incl. the Payne-Hanek big-argument reduction"
  lane a  t/thumb2/liba.c  t/thumb2/harnessa.c  "" 6  30 "thumb2 aligned(N)" \
    " = every aligned(N) global lands on its N after the link; 100+n names the first miss -- see t/thumb2/harnessa.c. the pad inside a section is laid by mooncc either way, so a miss here is sh_addralign: objsecs3's data lanes handing the linker a grain narrower than the stream asked for"
  lane f  t/thumb2/libf.c  t/thumb2/harnessf.c "" 16 30 "thumb2 bare floats" \
  ...
  lane z  t/thumb2/libz.c  t/thumb2/harnessz.c "-Ia/moon/include" 18 30 "thumb2 composites+varargs" \
...
thumb2sp)
  ...
  lane f  t/thumb2/libf.c  t/thumb2/harnessf.c "" 16 30 "thumb2sp bare floats" \
    " = AAPCS-VFP placement vs gcc -mfloat-abi=hard: s0..s15 with back-fill around the doubles, the stack past them, s0 for the return; 100+n names the first miss -- see t/thumb2/harnessf.c"
