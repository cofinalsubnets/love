#!/bin/sh
# t/gate/dialect.sh -- the x86 dialect lens (l/holo/dialect.l) against gcc's own two
# spellings: every t/cc file compiled with -masm=att and with -masm=intel must read to the
# same value. gcc is the oracle for what each dialect says; t/holo/dialect.l is the judge.
# skips without gcc.
#
# usage: dialect.sh LOVE OUTDIR      (from the repo root; LOVE is a word list, not a path)
set -u
love=$1 ho=$2
command -v gcc > /dev/null 2>&1 || { echo "t/holo/dialect: no gcc, skipped"; exit 0; }
d=$ho/.dialect
...
{ cat l/holo/holo.l l/holo/x64.l l/holo/dialect.l
  sed "s|@PAIRS@|$d/pairs|" t/holo/dialect.l
} | $love
