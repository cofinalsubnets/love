#!/bin/sh
# t/gate/decode.sh -- the x86 decoder (l/holo/decode.l) against objdump: every t/cc
...
# oracle for what the bytes say; t/holo/decode.l is the judge. skips without gcc or objdump.
#
# usage: decode.sh LOVE OUTDIR      (from the repo root; LOVE is a word list, not a path)
set -u
love=$1 ho=$2
command -v gcc > /dev/null 2>&1 || { echo "t/holo/decode: no gcc, skipped"; exit 0; }
command -v objdump > /dev/null 2>&1 || { echo "t/holo/decode: no objdump, skipped"; exit 0; }
command -v objcopy > /dev/null 2>&1 || { echo "t/holo/decode: no objcopy, skipped"; exit 0; }
d=$ho/.decode
mkdir -p $d
: > $d/pairs
for f in t/cc/*.c; do
...
{ cat l/holo/holo.l l/holo/x64.l l/holo/dialect.l l/holo/decode.l
  sed "s|@PAIRS@|$d/pairs|" t/holo/decode.l
} | $love
