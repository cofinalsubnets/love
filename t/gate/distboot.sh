#!/bin/sh
# t/gate/distboot.sh -- the release claim, stated over the artifacts.
#
...
#
# THE SEED IS THE TREE'S OWN b/love (seed-universal U2: the host build
...
( cd "$lean" && make -j"$(nproc 2>/dev/null || echo 4)" b/love ) > "$w/lean.log" 2>&1 \
  || { tail -20 "$w/lean.log"; fail "the source artifact does not build"; }
[ -x "$lean/b/love" ] || fail "the source build produced no love"
...
# compares this against leg 1's unbaked b/love. So the target is named here and CC
# with it -- which is the seed verb's own fallback spelled by hand (a/source.l names
...
( cd "$selfd" && PATH="$w/nocc:$PATH" LOVE_NO_IMAGE= \
    make -j"$(nproc 2>/dev/null || echo 4)" CC="$w/self/love mooncc" b/love ) \
  > "$w/selfb.log" 2>&1 \
...
# ---- 3. THE CLAIM ------------------------------------------------------------
# Both binaries here are the bare LINKS (the explicit b/love target, no
...
if cmp -s "$lean/b/love" "$selfd/b/love"; then
  echo "  OK both artifacts answer the SAME binary ($(wc -c < "$lean/b/love") bytes)"
else
  ls -l "$lean/b/love" "$selfd/b/love"
...
# compare selfpath against b/love), so running it whole costs a lay more and buys
...
# (a/source.l) supplies make/sh/sed and the rest out of the binary itself.
