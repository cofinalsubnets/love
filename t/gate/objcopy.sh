#!/bin/sh
# t/gate/objcopy.sh -- l/holo/copy.l against the tool it replaces. objcopy's two
...
{ echo "(borrow 'holo)"
  cat a/kore/text.l a/kore/u.l a/kore/asbook.l l/holo/thumb2.l \
      l/holo/elf.l l/holo/obj.l l/holo/link.l l/holo/copy.l
  echo "((cite 'holo 'objcopy) >argv)"; } > "$d/ocopy.l"

# the fixtures, ours end to end: mkfix.l lays one object and links it three ways
{ echo "(borrow 'holo)"
  cat a/kore/text.l a/kore/u.l a/kore/asbook.l l/holo/thumb2.l \
      l/holo/elf.l l/holo/obj.l l/holo/link.l t/gate/objcopy.l
...
"$ho/love" "$d/ocopy.l" -O binary t/gate/objcopy.sh "$d/mine.out" 2>/dev/null \
