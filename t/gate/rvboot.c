// t/gate/rvboot.c -- the riscv bring-up on a real hart: mkboot.l's sv39 lane and
// i/rv64/dtb.c's door, entered the way the kernel will be entered (qemu -M virt,
...
void kmain(void) {
  ...
  *(volatile uint64_t *) (k_hhdm + (uintptr_t) &witness) = 0xf00dcafef00dcafeull;
  ...
  // 5. the tree qemu built, read by i/dtb.h through that window: one bank, starting
