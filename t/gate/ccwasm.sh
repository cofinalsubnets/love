#!/bin/sh
# t/gate/ccwasm.sh -- the C battery on the WASM target, ccarch.sh's procedure with node
# as the machine: every t/cc/*.c built by `mooncc -t wasm`, run under node through
# i/wasm/run.mjs (the loader is the kernel), and required to answer exactly what the
...
# the OPTIONAL extra oracle, found as i/wasm/Makefile finds it
...
for f in t/cc/*.c; do
  ...
  timeout 60 "$NODE" i/wasm/run.mjs "$d/$b.wasm" > "$d/$b.tout" 2>&1; rt=$?
...
[ $n -gt 0 ] || fail "no programs ran from t/cc/"
if [ -n "$EMCC" ]; then
