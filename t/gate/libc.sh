#!/bin/sh
# t/gate/libc.sh -- the LIBC differential: our own C library against the
# system's, function by function, with gcc+glibc as the second opinion.
#
# each t/libc/*.c is built twice -- once by mooncc, which pulls
# a/moon/lib/moonlibc/ by need, and once by gcc against glibc -- run, and the
...
# IS fixed, so a legitimate difference never fails the gate. see t/libc/say.h.
#
...
n=0
fams=
for f in t/libc/*.c; do
  b=$(basename "$f" .c)

  moonrun -It/libc -o "$d/$b" "$f" > "$d/$b.build" 2>&1 \
  ...
  $cc_g -O0 -w -It/libc -o "$d/$b.g" "$f" 2> "$d/$b.gbuild" \
...
[ $n -gt 0 ] || fail "no programs found in t/libc/"

# ---------------------------------------------------------------------------
# phase 2: the headers must not PROMISE what the library cannot deliver.
#
# every function a/moon/include/{stdio,stdlib,string,ctype}.h declares is
...
names=$(grep -hE '^[a-z][a-zA-Z_0-9 ]*[ *][a-z_][a-z_0-9]*\(' \
          "$hdr/stdio.h" "$hdr/stdlib.h" "$hdr/string.h" "$hdr/ctype.h" "$hdr/signal.h" \
        | sed -E 's/\(.*//' | sed -E 's/.*[ *]//' | sort -u)
{
  echo '/* generated by t/gate/libc.sh -- do not edit */'
