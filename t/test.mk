# t/test.mk -- the test_* gates (and the uuwm/uukind corpus generators).
...
$(ho)/front: t/front/main.c $(R)/l/bare.c $(R)/i/alloc.c $(R)/i/horn.c $(love_h) $(ho)/liblove.a $(ho)/.hostcc $(R)/l/love_data.ld \
...
doc_tests = t/proto/dest.l t/proto/spl.l
test_doc: host
	@for s in $(doc_tests); do echo "TEST $$s"; \
	  cat t/00-init.l $$s | sh t/gate/run.sh doc "$m" ": ok" \
	    || { echo "  (the gate above is $$s)"; exit 1; }; \
	done
# native-codegen self-tests (the l/boot/glaze.l x86-64 jit): t/glaze-x86.l covers emit
...
ifeq ($a,x64)
test_glaze: host
	@echo TEST t/glaze-x86.l "(emit + auto)"
	@{ echo "(borrow 'holo)"; cat l/holo/x64.l l/holo/a64.l t/glaze-x86.l; } \
	  | sh t/gate/run.sh glaze "env LOVE_NO_IMAGE=1 $m" "t/glaze-x86:"
else
test_glaze:
...
test_hook: host
	@echo TEST t/glaze-hook.l "(the baked image)"
	@sh t/gate/run.sh -a hook "$m" "glaze-hook: ok" t/glaze-hook.l
	@echo TEST t/glaze-hook.l "(egg boot, hook.l out of the tree)"
	@sh t/gate/run.sh -a hook "env LOVE_NO_IMAGE=1 $m" "glaze-hook: ok" t/glaze-hook.l
else
test_hook:
...
test_glazebench: host
	@echo TEST t/gate/glazebench.sh "(the benches glaze through the driver)"
	@sh t/gate/glazebench.sh $m $a
else
test_glazebench:
	@echo "test_glazebench: skipped (the glaze emits for x64 / a64; host arch is $a)"
endif
# test_glazefuzz -- the glaze's differential fuzz (t/gate/glazefuzz.l): 3000 random closures
...
test_harp: host
	@$m t/gate/gates.l harp < /dev/null
...
test_gates: host
	@echo TEST t/gate/gates.l "(the app-law rows, one love)"
	@$m t/gate/gates.l < /dev/null
...
test_seat: host
	@echo TEST t/gate/seat.sh "(the file-seat lane)"
	@sh t/gate/seat.sh $m
...
# the front page's icon and fonts are laid (u/mkicon.l, u/mkfont.l) and checked in for
...
test_web: host
	@echo TEST u/mkicon.l u/mkfont.l
	@mkdir -p b/.w
	...
	@echo "  web: ok -- the icon and both fonts are what w/ lays"
...
test_moon: host $(love0)
	@sh t/gate/moon.sh $(ho) $m $(love0)
# the committed generated artifacts, laid from the tables that define them (l/mx.l the +/*
# matrices and the kind lattice, l/nifs.l the nif + instruction registry, quay.l the
...
# ...and the dependency, off the same roster: without it a new l/nifs.l row builds clean and
...
# sound because the generator IS l/nifs.l/mx.l and a stale love lays a fresh table. a tree
...
test_clay: host
	@echo TEST t/gate/clay.l "(clay G1: (cparse (clay-show c)) == c over t/cc)"
	@$m -l t/gate/clay.l < /dev/null
...
# test_moonfuzz -- moon's refusal surface: each t/cc file broken
...
test_moonfuzz: host
	@echo TEST t/gate/moonfuzz.l "(moon refusal fuzz: 8 mutants per file over t/cc)"
	@$m -l t/gate/moonfuzz.l < /dev/null
...
test_forge: host
	@echo TEST t/gate/forge.l "(forge: love IR -> holo -> nif -> differential)"
	@LOVE_NO_IMAGE=1 $m -l t/gate/forge.l < /dev/null
# every t/cc/*.c by `mooncc -t $*`, run under qemu-user, held to what x64 answers.
# the three programs no cross lane can build must refuse, not skip.
test_cca64 test_ccrv64: test_cc%: host
	@sh t/gate/ccarch.sh $* $(ho) $m
# test_ccwasm -- the same battery on the wasm target, node as the machine (ccwasm.sh)
test_ccwasm: host
	@sh t/gate/ccwasm.sh $(ho) $m
...
test_ccthumb1 test_ccthumb2: test_cc%: host
	@sh t/gate/ccthumb.sh $* $(ho) $m
# an outside corpus: c-testsuite's 220 programs held to the stdout the corpus ships.
# t/cc/ pins faults we had met; these did not, and found nine mooncc answers wrong --
...
# t/libc/*.c built by mooncc (pulling a/moon/lib/moonlibc/ by need) and by gcc, run,
# and the two outputs compared, so a drift names the function and the case.
test_libc: host
	@sh t/gate/libc.sh $(ho) $m
...
raw_env = gate_love_c='$(love_tu_c)' gate_host_c='$(host_c)' gate_arch_c='$(hosta_c)' gate_hosta='$(hosta)' gate_seat_c='$(R)/i/nokern.c $(R)/i/noblob.c'
test_raw: host
	@$(raw_env) sh t/gate/raw.sh x64 $(ho) $m $t
...
test_hdiff: host
	@echo TEST t/gate/hdiff.sh
	@sh t/gate/hdiff.sh gcc clang
...
test_dtb:
	@echo TEST t/gate/dtb.c
	@$(CC) -I$R/i -I$R/l -I$R -o $(ho)/.dtbgate $R/t/gate/dtb.c
	@$(ho)/.dtbgate
...
# Three objects and nothing else -- the stub, the door, and t/gate/rvboot.c standing in
...
test_fixpoint: host $(moon_seat_o) $(love0) b/mooncc0.image
	...
	@gate_love_c='$(love_tu_c)' gate_host_c='$(host_c)' gate_arch_c='$(hosta_c)' \
	  ...
	  sh t/gate/fixpoint.sh $(ho) $(love0) $(hosta) $(moon_d) $(moon_o) $(moon_seat_o) $(kart_o)
...
.PHONY: test_fat
test_fat: dist-fat
	@sh t/gate/fat.sh $(fat) $a $(xa) $(xqemu) "$(love0)" $(ho) $(xd) $(uname_$(xa))
...
test_raw_bake: test_raw
	@sh t/gate/raw-bake.sh $(ho) $t
# test_rv64 -- the t/cc battery `mooncc -t rv64` under qemu-riscv64, exit code
...
test_raw_rv64: host
	@$(raw_env) sh t/gate/raw.sh rv64 $(ho) $m $t
...
# in C/byte order: t/uu.l defines the kernel t/uukindlaw.l calls. Opt-in; needs qemu.
# t/a64/callout.l rides past $t: it builds 'a64 nifs and runs them, so only an a64
# love may read it -- gate_sentinel is how the gate knows it was read and not stopped short.
test_raw_a64: host
	@$(raw_env) gate_sentinel='t/a64/callout:.* ok' \
	  sh t/gate/raw.sh a64 $(ho) $m $t t/a64/callout.l
# the ELF32/EM_ARM object writer (l/holo/obj.l objsecs32) and the 32-bit data model:
...
#   virt      the bare rv64 hart (i/virt/)
...
# for its C_API headers and for pdc. See t/gate/playdate.sh for what the image is held to.
test_playdate: host
	@sh t/gate/playdate.sh "$(MAKE)" $m
...
test_bakerep: host
	@echo TEST t/gate/bakerep.sh
	@sh t/gate/bakerep.sh $(ho)
test_distboot: dist
...
test_gz: host
	@echo TEST t/gate/gzfind.l
	@$m $R/t/gate/gzfind.l
	@echo TEST t/gate/targz.sh
	@sh t/gate/targz.sh $(ho)/love
# test_root -- the privileged verbs: chroot, mount, umount, sync, mkfifo, mknod. the
# refusals as an ordinary user, then t/gate/rootns.l, which makes itself root in an
...
test_root: host
	@echo TEST t/gate/root.sh
	@sh t/gate/root.sh $(ho) $(ho)/love
...
# word. t/fat.l proves the filesystem's own laws over a cask, needing nothing
...
test_fat32: host
	@echo TEST t/gate/fat32.sh
	@sh t/gate/fat32.sh $(ho) $(ho)/love
...
test_cpio: host
	@echo TEST t/gate/cpio.sh
	@sh t/gate/cpio.sh $(ho)/love
# The neutral assembler (l/holo/) + its x86-64 backend: every encoder golden is
# objdump-checked (t/holo/golden.l). a host-only app -- it adds no nif and is not
...
test_holo: host
	...
	@cat l/holo/holo.l l/holo/x64.l l/holo/a64.l l/holo/thumb2.l \
	    ...
	    l/holo/wasm.l l/holo/wasmfn.l t/holo/golden.l | sh t/gate/run.sh holo "$m" ", 0 failed"
...
# the bytes back (decode.sh judges it against objdump). t/holo/as.l's goldens are
...
test_as: host
	@echo TEST t/holo/as.l
	@cat l/holo/holo.l l/holo/x64.l l/holo/dialect.l l/holo/decode.l l/holo/as.l t/holo/as.l \
	  | sh t/gate/run.sh as "$m" ", 0 failed"
	@sh t/gate/asrefuse.sh "$m"
	@sh t/gate/dialect.sh "$m" $(ho)
	@sh t/gate/decode.sh "$m" $(ho)
...
# t/holo/golden.l pins the header fields; this pins the only opinion that counts. Needs qemu-arm
# and nothing else -- no as, no ld, no arm-none-eabi -- so it runs where the thumb gates skip.
test_elf32: host
	@sh t/gate/elf32.sh $(ho)
...
vclean = rm -f $(foreach n,$1,t/proof/rocq/$n.vo t/proof/rocq/$n.vok t/proof/rocq/$n.vos t/proof/rocq/$n.glob t/proof/rocq/.$n.aux)
dclean = rm -f $(foreach n,$1,t/proof/rocq/$n_ref.ml t/proof/rocq/$n_ref.mli t/proof/rocq/$n_drive)

ifeq ($(COQC),)
test_proof test_gc test_gen test_uugen test_mx:
	@echo "$@: skipped (needs rocq/coqc)"
else
# spec.v -- love's headline laws (the numeral / function / absence core of t/spec.l) as Rocq
...
# Import spec`. /warn one spelling everywhere (`cd t/proof/rocq && -R . ""`), or spec.vo's logical
# name is not the one gen.v asks for. a static pattern: big/mx/enc take their own flags.
rocq_kept = t/proof/rocq/spec.vo t/proof/rocq/patch.vo
$(rocq_kept): t/proof/rocq/%.vo: t/proof/rocq/%.v
	@echo TEST t/proof/rocq/$*.v "(coqc)"
	@cd t/proof/rocq && $(COQC) -q -R . "" $*.v
...
test_gc:
	@echo TEST t/proof/rocq/gc.v "(coqc)"
	@$(COQC) -q t/proof/rocq/gc.v
	@$(call vclean,gc)
# The .l -> .v pipeline: u/spec2coq.l reads t/spec.l and emits gen.v, the spec generating
# theorems for its own numeral facts. Regenerated every run, so asserts and proofs cannot diverge.
test_gen: host $(rocq_kept)
	@echo 'LOVE	't/proof/rocq/gen.v "(u/spec2coq.l on $m)"
	@$m u/spec2coq.l > t/proof/rocq/gen.v
	@echo TEST t/proof/rocq/gen.v "(coqc, against spec.v's shared model)"
	@cd t/proof/rocq && $(COQC) -R . "" gen.v
	@$(call vclean,gen)
...
test_uugen: host
	@echo 'LOVE	't/proof/rocq/uugen.v "(u/uu2coq.l on $m)"
	@$m u/uu2coq.l > t/proof/rocq/uugen.v
	@echo TEST t/proof/rocq/uugen.v "(coqc)"
	@$(COQC) -q t/proof/rocq/uugen.v
	@$(call vclean,uugen)
...
test_mx: host
	@echo TEST t/proof/rocq/mx.v "(the dispatch matrices: band factorization + dispatch commutativity, coqc)"
	@cat l/mx.l u/mx2coq.l | $m > t/proof/rocq/mx.v
	@cd t/proof/rocq && $(COQC) -q mx.v >/dev/null
	@$(call vclean,mx)
endif
...
test_encver: host
	@echo TEST t/proof/rocq/enc.v t/proof/rocq/encmem.v t/proof/rocq/encli.v "(coqc round-trip proofs -> ocaml refs vs holo, byte-exact)"
	@cd t/proof/rocq && $(COQC) -q enc.v >/dev/null && $(COQC) -q encmem.v >/dev/null && $(COQC) -q encli.v >/dev/null \
...
test_holofuzz: host
	@echo TEST t/holo/fuzz/fuzz.l "(holo x64+a64+riscv encoder differential fuzz)"
	@FUZZ_N=8 FUZZ_SEED=20250717 $m t/holo/fuzz/fuzz.l \
	  || { echo "FAIL holofuzz -- a holo encoding disagrees with its disassembler"; exit 1; }
	@if command -v llvm-mc >/dev/null 2>&1; then \
	   $m t/holo/fuzz/sysdiff.l \
...
#   uukind    the abstract kinds-lattice join         -> t/uukindlaw.l, the semilattice laws
#   uuhomgen  dest.l's two code generators, on its law sites -> t/uuhomlaw.l, the destination-die laws
#   uusplgen  spl.l's three call-site compilers (call, binding splice, substitution
#             splice) on its samples                  -> t/uuspllaw.l, the splice license
#   uumx      love.c's +/* dispatch matrices (l/mx.l) -> t/uumxlaw.l, the band lattice
#   uuvallaw  CLAUDE.md's laws off t/law.l's own rows -> proved where they stand, one
#             spelling for the fuzz lane and the proof lane both
$(eval $(call uu_corpus,uuwm,uuwmgen,a/lux/core.l))
$(eval $(call uu_corpus,uukind,kinds2uu,t/proto/kinds.l))
$(eval $(call uu_corpus,uuhomgen,dest2uu,t/proto/dest.l))
$(eval $(call uu_corpus,uusplgen,spl2uu,t/proto/spl.l))
$(eval $(call uu_corpus,uumx,mx2uu,l/mx.l))
$(eval $(call uu_corpus,uuvallaw,law2uu,t/law.l))
...
# wget against a live https peer: opt-in, it needs the internet (t/host/wgetnet.l)
test_wgetnet: host
	@echo TEST t/host/wgetnet.l "(wget over TLS 1.3 to a live peer)"
	@cat t/00-init.l t/host/wgetnet.l | sh t/gate/run.sh wgetnet "$m" "wgetnet: ok"
...
test_doomx: $(R)/t/host/doomx.l
	@$(MAKE) -s host DOOM=1
	@echo TEST t/host/doomx.l "(doom on X, 300 frames under Xvfb, a held key)"
	@cat t/00-init.l t/host/doomx.l | sh t/gate/run.sh doomx "$m" "doomx: ok"
