(* oracle_drive.ml -- drives the EXTRACTED reference normalizer (normalizer.ml,
   coqc'd from t/proof/rocq/extract.v on spec.v's proven subst/shift) against ev.
   ...
   independent, see t/oracle.l). The love binary runs the emitted program;
...
(* --- random closed affine generator (port of t/oracle.l's `gen`) --- *)
