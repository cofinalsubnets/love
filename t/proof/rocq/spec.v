(* proof/spec.v -- ai's headline laws, machine-checked in Rocq.
   ...
   The executable spec is t/spec.l: every claim assert-backed, on every
   ...
   "up to explosion of world": the uu kernel (t/uu.l) runs type-in-type
...
(* false is NOTHING: a value is false iff its net measure is <= 0. The net is
   ...
   The FINER carrier is t/uuval.l's `val`, in uu: the measure there is complex
...
(* `=` is alpha+structural and deliberately stops there (above). The runtime's
   ...
   assertion against the real binary in t/spec.l (e.g. ((\ x x) 5) ; 5): the
...
(* THE TWO COUNT LAWS SPLIT (the associativity arc): `*` repeats by |count|
   ...
   and its third face is the n-ary closure's fire-vs-hold (t/oracle.l,
   t/proof/rocq/extract.v). (Only the COMPOSITION count saturates; the
...
(* ============================================================ *)
(* the Z lane: ONE model, shared with the generated t/proof/rocq/gen.v  *)
(* ============================================================ *)
(* The nat `app` (Nat.pow) above carries the numeral LAWS by clean unary induction.
   The GENERATED corpus checks (t/proof/rocq/gen.v, from u/spec2coq.l) instead need Z --
