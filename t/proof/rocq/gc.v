(* t/proof/rocq/gc.v -- the generational MINOR is SOUND: under a complete write barrier,
   ...
   theorems t/host/gcpause.l's gauge instance-checks). and the COPY LOOP has
...
(* a copying collector's pause IS its copy volume, and the minor copies exactly
   ...
   (barrier_sound says that scan covers everything live). t/host/gcpause.l
...
(* axiom-free, like the rest of t/proof/rocq/: every result is closed under the global
