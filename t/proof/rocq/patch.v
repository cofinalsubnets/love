(* proof/patch.v -- the PATCH GROUPOID, the hand-proven slices, machine-checked in Rocq.
   ...
   landed them as uu proof terms (t/uupatch.l), which u/uu2coq.l +
   u/uu2lean.l emit into t/proof/rocq/uugen.v AND t/proof/lean/uugen.lean --
   ...
   (t/uupatch.l's ladder). The hunk record and splice STAY here as the
   ...
   OPEN: the named-slot slice went first (t/uupatch.l -- the laws as uu proof
   TERMS, isdeceqnat's coprod carrying the evidence Nat.eqb dropped, emitted to
   both kernels by u/uu2coq.l + u/uu2lean.l and audited axiom-free), and
...
(* to the uu leg -- t/uupatch.l -> uugen.v / uugen.lean)      *)
(* ============================================================ *)

(* Mirror of t/patch.l's second form's model. A state is a SEQUENCE of
...
(* Mirror of t/patch.l's third form, on the lawful region (the .l's pull is
...
(* Mirror of t/patch.l's fourth form, on the lawful part. The .l hashes
...
(* Mirror of t/patch.l's fifth form, the algebra half. A patch is a hunk
...
(* Mirror of t/patch.l's sixth form, the L1 core at ELEMENT grain: the deep
...
(* Mirror of t/patch.l's seventh form, the cell algebra: two co-valid
...
(* Mirror of t/patch.l's eighth form, in its two genuinely new halves.
...
(* Mirror of t/patch.l's ninth form. Terms and the matcher are the
