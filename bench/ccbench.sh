...
# globs (l/love.c, inle/*.c, apps/...), so every output/include path below must be absolute.
...
# l/, and the host set is inle/ less the kernel's own six
...
build_mooncc() { # $1=binpath
  ...
  ( cd "$R" || exit 1
    for b in $love_tu; do
      mc -D ai_tco=1 -D AiHaveVersionH -Iout -I. -Il -Iinle -Iout/lib -c "l/$b.c" "$od/$b.o" || exit 1; done
    for f in $host_cs; do b=$(basename "$f" .c)
