...
case $lang in
  # love: the native glaze (src/core/boot/glaze/{emit,auto}.l) is BAKED into out/love and active by default, so the
  ...
  love)            ext=l;    bin=../out/love;  cmd='cat $([ "$b" = cdcl ] && printf "%s " ../src/apps/sat/sat.l) bench.l benches/$b.l | LOVE_NO_IMAGE=1 ../out/love' ;;
