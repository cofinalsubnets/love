  ...
  struct {
   ...
   union {
    ...
    struct ai_io {
     ...
     // three words: prel's tap/jug poke this layout by index (core/boot/prel.l), so a word
...
// the boot driver: ai_egg_(g, egg, p1, corpus) applies core/boot/egg.l to the quoted corpus
...
// named one is registered in g->kreg (name -> (serial . table)) by core/boot/post.l's `coin`.
