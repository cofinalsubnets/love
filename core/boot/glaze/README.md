...
> - **`core/boot/glaze/emit.l`** — a love-level **x86-64 emitter**: compiles `(\ x E)` arithmetic
...
>   exactly the 62-bit fixnum boundary). x86-64 only; load with `-l core/boot/glaze/emit.l`.
>
...
> executable handle) and `core/boot/glaze/probe.l` predated `nat` and were superseded by it;
...
A retired probe (`core/boot/glaze/probe.l`, deleted with `eat`/`toast`) built a buf holding
six AMD64 bytes —
...
forever. `make test_glazefuzz` (`core/boot/glaze/fuzz.l`) is the standing guard: 3000 random
