...
static unsigned char __sigsi[64];        /* did this row ask for SA_SIGINFO? */
/* both kernels hand a trampoline three arguments, so the shim takes three
...
int sigaction(int sig, struct sigaction const *a, struct sigaction *old) {
  if (__ai_osv == 2) {
    ...
    unsigned char prevsi = __sigsi[fs];
    if (a) {
